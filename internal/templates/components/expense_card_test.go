...
func mustRenderExpenseCard(e database.Expense, lang string) string {
	buf := &bytes.Buffer{}
	err := components.ExpenseCard(e, lang).Render(context.Background(), buf)
	if err != nil {
		panic(err)
	}
	return buf.String()
...
func TestExpenseCard_ShowsCategory(t *testing.T) {
	...
	if !strings.Contains(output, "Utilities") {
		t.Error("expected category 'Utilities' (translated) in expense card output")
	}
}
...
func TestExpenseCard_ShowsVisibility(t *testing.T) {
	...
	output := mustRenderExpenseCard(e, "en")
	if !strings.Contains(output, "Hidden") && !strings.Contains(output, "hidden") && !strings.Contains(output, "private") {
...
func TestExpenseCard_ShowsVisibleOnly(t *testing.T) {
	...
	output := mustRenderExpenseCard(e, "en")
	if !strings.Contains(output, "yellow") && !strings.Contains(output, "Visible") && !strings.Contains(output, "visible") && !strings.Contains(output, "read") {
...
func TestExpenseCard_Triangulation_DifferentDescriptions(t *testing.T) {
	...
	out1 := mustRenderExpenseCard(e1, "en")
	out2 := mustRenderExpenseCard(e2, "en")
...
func TestExpenseCard_SpanishCurrencyFormat(t *testing.T) {
	e := database.Expense{ID: 1, Description: "Luz", Amount: 1234.56, Category: "utilities", Date: time.Date(2026, 1, 2, 0, 0, 0, 0, time.UTC), Visibility: database.VisibleEditable}
	output := mustRenderExpenseCard(e, "es")
	if !strings.Contains(output, "$1.234,56") {
		t.Errorf("expected Spanish currency '$1.234,56', got %s", output)
	}
}

func TestExpenseCard_SpanishDateFormat(t *testing.T) {
	e := database.Expense{ID: 1, Description: "Dinner", Amount: 45.00, Category: "food", Date: time.Date(2026, 1, 2, 0, 0, 0, 0, time.UTC), Visibility: database.VisibleEditable}
	output := mustRenderExpenseCard(e, "es")
	if !strings.Contains(output, "2 de enero de 2026") {
		t.Errorf("expected Spanish date '2 de enero de 2026', got %s", output)
	}
}

func TestExpenseCard_SpanishVisibilityLabel(t *testing.T) {
	e := database.Expense{ID: 1, Description: "Gift", Amount: 30.00, Category: "other", Date: time.Date(2026, 7, 10, 0, 0, 0, 0, time.UTC), Visibility: database.VisibleEditable}
	output := mustRenderExpenseCard(e, "es")
	if !strings.Contains(output, "Visible y editable") {
		t.Errorf("expected Spanish visibility 'Visible y editable', got %s", output)
	}
}

func TestExpenseCard_StoredValuesUntouched(t *testing.T) {
	e := database.Expense{ID: 1, Description: "Groceries", Amount: 50.00, Category: "food", Date: time.Date(2026, 7, 27, 0, 0, 0, 0, time.UTC), Visibility: database.VisibleEditable}
	_ = mustRenderExpenseCard(e, "es")
	// Stored fields must not be mutated by locale rendering
	if e.Category != "food" {
		t.Error("stored category was mutated by locale rendering")
	}
	if e.Amount != 50.00 {
		t.Error("stored amount was mutated by locale rendering")
	}
	if e.Visibility != database.VisibleEditable {
